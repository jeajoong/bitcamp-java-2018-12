...
// 4) Log4J 설정법
//    => CLASSPATH 루트 패키지에 log4j.projecties 이름으로 설정 파일을 둔다
//  1) 출력 등급 설정
...
//      INFO : 애플리케이션의 주요 실행 정보.     예) 어떤 IP의 클라이언트의 접속 정보, 연결 종료
//      DEBUG : 애플리케이션의 내부 실행 상태를 추적해 볼 수 있는 정보.    
//                      예) Mybatis의 SQL을 실행하는 SQL, SQL을 실행할 때 in-parameter에 설정되는 값
//      TRACE : 디버그 보다 더 상세한 정보.
//                      예) IoC 컨테이너가 생성한 객체 목록
//
// => 등급에 따른 출력 제어 
//       FATAL > ERROR > WAPN > INFO > DEBUG > TRACE
//       - 지정된 등급 이상의 로그는 모두 출력된다.
//       - 즉 INFO 등급의 로그만 출력하도록 했으면, INFO 보다 큰 레벨(WAPN, ERROR, FATAL)의 로그도 출력한다.
//  
//  2) 출력 담당자 지정
//  => 어디로 출력할 것인지 지정한다.
...
//          - HTML 태그로 출력.
//      org.apache.lig4j.PatternLayout
...
//  => 문법
//        %p               - 출력 등급. 예) FATAL, ERROR, WAPN, INFO, DEBUG, TRACE
